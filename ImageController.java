...
import com.webjjang.image.service.*; // service 패키지 안에 있는 모든 클래스를 다 import 함 : * - 모두다 
...
public class ImageController implements Controller {
	...
	public void execute() {
		...
		while(true) { 
			...
			//메뉴출력,입력
			Output.menu("1.이미지 게시판 리스트 2.이미지 게시판 글보기 3.이미지 게시판 글쓰기 ","4.이미지 게시판 글수정 5.이미지 게시판 글삭제 0.이전메뉴",
			...
			case "3" :
				...
				invo.setId(Input.getString("아이디"));
			...
			case "4" :
				//1.수정 할 글번호를 받음				
				//2.수정 할 데이터를 DB에서 가져와서 - ImageViewService
				//가져온 데이터를 2번 이상 사용 할 때는 반드시 변수에 저장해서 사용할 것 
				ImageVO updateVO =((ImageVO)ExeService.execute(new ImageViewService(),Input.getLong("수정할 글 번호 ")));
				//3.리스트 출력 -글보기 출력-> 수정처리 while안에서 출력 
				//4.수정 처리를 함 
				updateImageVO(updateVO); //위의 메소드 안에서 vo객체를 변경 하면 밖에서도 변경됨 (참조형 변수의 특징!)
				System.out.println("ImageController.execute() 수정 완료 후 수정 메소드 밖에서의 vo " + updateVO);
				//5.수정 내용을 DB에 저장  -ImageUpdateService
				int updateResult = (Integer)ExeService.execute(new ImageUpdateService(), updateVO);
				//6.수정 결과 출력 -print(result,msg)
				ImagePrint.print(updateResult,"이미지 게시판 글 수정");
				break;
			case "5" :
				//삭제 할 글 번호를 받는다
				//DB에서 삭체처리 -ImageDeleteService
				int deleteResult =(Integer)ExeService.execute(new ImageDeleteService(),Input.getLong("삭제 할 글 번호 "));
				ImagePrint.print(deleteResult,"이미지 게시판 글 삭제");
				break;
			case "0" :
				//이전메뉴 
				return;
